...
ボードの列は進捗ではなく **AIへの指示** を表します。

| レーン | 意味 |
| --- | --- |
| `受信トレイ (triage)` | 新規。人間の判断待ち（初期値）。 |
| `AIで修正 (ai-fix)` | **AIの作業キュー＝発注**。AIはこのレーンだけを対応する。 |
| `自分で対応 (manual)` | 人間が対応。AIは触らない。 |
| `対象外 (wontfix)` | 理由つきで却下。今後通知しない。 |
| `完了 (done)` | 再レビューで解消を確認済み。 |
